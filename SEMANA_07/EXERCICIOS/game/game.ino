        ...
        if (c == '\n') {                    // se o byte é um caractere de nova linha
          ...
          if (currentLine.length() == 0) {
            ...
            // Mostrar a pagina HTML em si
               client.println(content='width=device-width, initial-scale=1.0'><title>Cara ou coroa!</title></head>")
          ...
          long interval = random(1000, 10000); 
          unsigned long previousMillis = millis(); 
          unsigned long currentMillis = millis(); 
          
          digitalWrite(LED_CARA, LOW);
          digitalWrite(LED_COROA, LOW);

           if (digitalRead(LED_COROA) == LOW) {
                digitalWrite(LED_CARA, HIGH); 
          if (digitalRead(LED_CARA) == LOW) {
              digitalWrite(LED_COROA, HIGH);                 
